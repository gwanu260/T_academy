...
- method
    - HTTP 프로토콜상에서 클라이언트가 서버측으로 데이터를 전달하는 방식
        - get, head      : 프로토콜의 헤더(크기고정)를 통해 전달 
                           -> 소량 -> 간단한 키값, 특정값, 유출되도 문제없는 데이터만
                           -> 아무것도 않보내도 됨(기본 get), 링크
...
# 기본 라우팅 => get방식으로 서버측에 요청
@app.get("/")
def home():    
    return { "msg":"hello fastapi@@"}

# GET방식 요청 + 
# 경로(path) 매개변수(url에 데이터를 심어서 전달)로 데이터를 전달한다
# http://127.0.0.1:8000/items/1a%EA%B0%80!
# http://127.0.0.1:8000/items/1a가! => 문자열로 전달됨
@app.get("/items/{id}")
